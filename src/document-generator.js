function generateRequirementsMarkdown(context) {
  return [
    ...
    '',
    '1. Prioritize zero-trust secret handling and security hygiene over all other concerns.',
    '2. Prioritize execution speed while keeping the workflow deterministic.',
    '3. Ensure reruns are idempotent.',
    '4. Produce verbose audit logging for traceability.',
    '5. Minimize external API usage where possible without reducing correctness.',
    '6. Keep the workflow resilient enough to retry transient failures and fail safely after logging when external dependencies remain unavailable.',
    '7. Preserve maintainability through small, modular changes and clear separation between detection, generation, sync, verification, and PR steps.',
    '',
    '## Assumptions',
    '',
...
function generateDocumentSet(context) {
  return [
    {
      path: 'generatedDocs/requirements.md',
      body: generateRequirementsMarkdown(context),
    },
    {
      path: 'generatedDocs/architecture.md',
      body: generateArchitectureMarkdown(context),
    },
    {
      path: 'generatedDocs/design-review.md',
      body: generateDesignReviewMarkdown(context),
    },
    {
      path: 'generatedDocs/impl-plan.md',
